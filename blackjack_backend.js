  ...
  function updateScore(username, bothStand, sessId) {
    ...
    if (bothStand) {
      ...
      if (dealerScore[sessId] > playerScore[sessId]) {
        gameState[sessId] = "Since you and the dealer both took a stand, the game is over. The dealer's final score is " + dealerScore[sessId] + " and your final score is " + playerScore[sessId] + ". You lose.";
      } else if (playerScore[sessId] > dealerScore[sessId]) {
        gameState[sessId] = "Since you and the dealer both took a stand, the game is over. The dealer's final score is " + dealerScore[sessId] + " and your final score is " + playerScore[sessId] + ". Congratulations, you win!";
        win[sessId] = true;
      } else if (playerScore[sessId] === dealerScore[sessId]) {
        gameState[sessId] = "Since you and the dealer both took a stand, the game is over. The dealer's final score is " + dealerScore[sessId] + " and your final score is " + playerScore[sessId] + ". The game ends in a tie.";
        tie[sessId] = true;
      }
    } else {
...
var blackjack_backend = (function() {
  ...
  function deal(username, sessId) {
    dealerHand.push(randomCard(deck, sessId), randomCard(deck, sessId));
    playerHand.push(randomCard(deck, sessId), randomCard(deck, sessId));
    updateScore(username, false, sessId);
  }
  ...
  function moveHit(url) {
    let sessId = url.slice(url.indexOf('sessId=') + 7, url.indexOf('&'));
    let username = url.slice(url.indexOf('username=') + 9);
    playerHand.push(randomCard(deck, sessId));
    updateScore(username, false, sessId);
    return {
      gameOver: gameOver,
      gameState: gameState,
  ...
  function moveStand(url) {
    ...
    while (dealerScore < 17) {
      dealerHit++;
      dealerHand.push(randomCard(deck, sessId));
      updateScore(username, false, sessId);
  ...
  function initGame(url) {
    ...
    gameOver = false;
    for (var i = 0; i < cardNums.length; i++) {
      deck.push(cardNums[i] + " of Clubs", cardNums[i] + " of Spades", cardNums[i] + " of Diamonds", cardNums[i] + " of Hearts");
    }
    deal(username, sessId);
    return {
      playerHand: playerHand,
